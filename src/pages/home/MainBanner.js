...
function MainBanner() {
  const { thisSeason, isSearch, loading } = useGlobalContext();
  ...
  useEffect(() => {
    ...
    const firstTitle = thisSeason ? thisSeason[0]?.title : "";
    setTitles(firstTitle);

    const firstPlot = thisSeason ? thisSeason[0]?.synopsis : "";
    setPlot(firstPlot);
  }, [swiper1Ref.current, swiper2Ref.current, thisSeason]);
                  ...
                  {plot && plot.length > 380 && (
                    <button
                      className={`btnShowMore ${
                        window.innerWidth >= 1200 ? "minWidth1200" : ""
                      }`}
